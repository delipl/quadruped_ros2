...
controller_interface::CallbackReturn QuadrupedController::on_configure(
  ...
  try {
    ...
    foot_position_rt_pub_ =
        std::make_unique<DataRTPublisher>(foot_position_normal_pub_);
    ...
    visualization_rt_pub_ =
        std::make_unique<VisualizationRTPublisher>(visualization_normal_pub_);

    visualization_ =
        std::make_unique<Visualization>(get_node(), "~/visualization_markers");

    multi_dof_state_normal_pub_ =
        get_node()->create_publisher<MultiDofStateStampedMsg>(
    ...
    multi_dof_task_state_rt_pub_ = std::make_unique<MultiDofStateStampedRTP>(
        multi_dof_task_state_normal_pub_);

    step_service_ =
        get_node()->create_service<example_interfaces::srv::AddTwoInts>(
            "~/step",
...
QuadrupedController::update(const rclcpp::Time &time,
  ...
  target_foot_positions_ << msg->fl_foot_position.x, msg->fl_foot_position.y,
      msg->fl_foot_position.z, msg->fr_foot_position.x, msg->fr_foot_position.y,
      msg->fr_foot_position.z, msg->rl_foot_position.x, msg->rl_foot_position.y,
      msg->rl_foot_position.z, msg->rr_foot_position.x, msg->rr_foot_position.y,
  ...
  for (std::size_t i = 0; i < legs_map_.size(); ++i) {
    ...
    foot_positions_error_.segment<3>(i * 3) =
        target_foot_positions_.segment<3>(i * 3) -
        foot_positions_.segment<3>(i * 3);

    foot_control_positions_.segment<3>(i * 3) =
        target_foot_positions_.segment<3>(i * 3);

    // z coordinate  PID
    foot_control_positions_[i * 3 + 2] +=
        params_.pid_z_leg_trajectory * foot_positions_error_[i * 3 + 2];

    foot_control_positions_[i * 3 + 2] =
        std::max(foot_control_positions_[i * 3 + 2], -0.22);
    ...
    for (std::size_t j = 0; j < 3; ++j) {
      if (std::isnan(q[j])) {
        ...
        RCLCPP_WARN(get_node()->get_logger(),
                    "Inverse kinematics failed for leg %zu joint %zu", i, j);
      } else {
  ...
  target_joint_efforts_ = Kp.cwiseProduct(joint_positions_errors_) +
                          Kd.cwiseProduct(joint_velocity_errors_) +
                          feed_forward_;
  ...
  for (size_t i = 0; i < legs_map_.size(); ++i) {
    ...
    legs_map_[i].update_passive_joints_dynamics(joint_states[2].position,
                                                joint_states[2].velocity,
    ...
    vis_msg.markers.push_back(
        visualization_->createArrow(start, target_p, 0.05, red, frame_id, i));
    ...
    vis_msg.markers.push_back(visualization_->createArrow(
        start, target_q2, 0.05, blue, frame_id, i + 4));
    ...
    vis_msg.markers.push_back(visualization_->createArrow(
        start, target_bar, 0.05, green, frame_id, i + 8));
  }
  ...
  RCLCPP_DEBUG(get_node()->get_logger(),
               "Update loop elapsed time: %ld microseconds", elapsed_time);

  return controller_interface::return_type::OK;
}

std::size_t QuadrupedController::get_state_interface_index(
    std::size_t leg_index, std::size_t joint_index,
